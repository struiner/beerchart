...
for (const match of text.matchAll(objectPattern)) {
  ...
  const field = (name) => object.match(new RegExp(`"${name}"\\s*:\\s*(?:"([^"]*)"|(null))`))?.[1] ?? null;
...
const source = readFileSync('src/app/core/data/beer-taxonomy-entries.ts', 'utf8');
const entries = [...source.matchAll(/"id"\s*:\s*"(style:[^"]+)"[\s\S]*?"title"\s*:\s*"([^"]+)"/g)].map((match) => ({ id: match[1], title: match[2], slug: slug(match[2]) }));
const entrySlugs = new Set(entries.map((entry) => entry.slug));
const links = [...new Set(records.map((record) => record.linked).filter(Boolean))];
writeFileSync('brand-pdf-summary.json', JSON.stringify({
